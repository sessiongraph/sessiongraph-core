...
## FIXED — Round 3 (CONNECT deadlock, MITM, shutdown safety, chart/compression math)

### C4. CONNECT handler deadlock ✅
**Root cause:** `hyper::upgrade::on(req).await` in `connect_handler` blocks forever because the upgrade future can only resolve after the `200 OK` response has been sent back to the client. Awaiting the upgrade in the handler before returning a response creates a deadlock.

**Fix:** Pass the upgrade future to a spawned task instead of awaiting it in the handler. The handler returns `200 OK` immediately so the upgrade can complete.

### C5. setx/reg failures silently discarded ✅
**Root cause:** `set_proxy_env_vars()` and `remove_proxy_env_vars()` used `Output::wait()` which discards exit codes. Failures to set or remove environment variables were invisible.

**Fix:** Changed to `wait_with_output()` and log exit codes via `tracing::warn!` on failure.

### C6. ProxyShutdown::drop panic risk ✅
**Root cause:** `ProxyShutdown::drop` used `Handle::current()` to get the tokio runtime, which panics if the runtime is already dropped during app shutdown.

**Fix:** Use `Handle::try_current()` instead. Also reordered cleanup to end sessions first (while runtime is alive), then clean up env vars.

### C7. Token chart re-fetched every 5 seconds ✅
**Root cause:** Chart fetch effect depended on `stats?.today?.requests` which changes every poll cycle (5s), causing unnecessary re-fetches.

**Fix:** Changed dependency to `new Date().toISOString().slice(0, 10)` so it only re-fetches when the day changes.

### C8. Compression stat averaged over all sessions including zero-raw ✅
**Root cause:** The compression percentage averaged `compression_ratio` over all active sessions, including sessions with zero raw tokens (ratio=1), inflating the average.

**Fix:** Filter out sessions with zero `tokens_in_raw` before computing the average.
...
**Frontend: clean.** `pnpm typecheck` and `pnpm build` pass.
**Tests: 68/68 pass.**
